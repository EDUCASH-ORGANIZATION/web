...
import { verifyStudent, rejectStudent, resetRejection } from "@/lib/actions/admin.actions"
...
function RejectModal({ userId, name, onClose }) {
  ...
  async function handleReject() {
    ...
    if (result.error) {
      ...
      toast({ message: `Dossier de ${name} rejeté.`, type: "info" })
      onClose()
      router.refresh()
    }
  }
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <textarea
            ...
            placeholder="Ex : Carte étudiante illisible, établissement non reconnu, document expiré…"
            rows={4}
          ...
          <p className="text-[11px] text-gray-400">
            Ce motif sera envoyé à l&apos;étudiant par email.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {[
            ...
            <button
              ...
              type="button"
              onClick={() => setReason(s)}
              className="text-[11px] px-2.5 py-1 rounded-full border border-gray-200 text-gray-500 hover:border-red-300 hover:text-red-600 transition-colors"
            >
              {s}
            </button>
          ))}
...
function VerificationCard({ profile }) {
  ...
  const borderCls = profile.is_verified
    ? "border-green-200"
...
export function VerificationsTabs({ submitted, incomplete, rejected, verified, all }) {
  const [activeTab, setActiveTab] = useState("submitted")

  const lists = { submitted, incomplete, rejected, verified, all }
