...
import { Send, CheckCircle, Loader2, ShieldAlert } from "lucide-react"
...
/**
 * Formulaire de candidature — état calculé côté serveur, soumission côté client.
 *
 * @param {{
 *   missionId: string,
 *   budget: number,
 ...
 * }} props
 */
export function StudentApplyForm({
  missionId,
  budget,
  clientName,
  estimatedDuration,
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 flex flex-col gap-5 sticky top-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500">
          Proposée par <span className="font-semibold text-gray-700">{clientName}</span>
        </p>
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        <textarea
          ...
          rows={5}
          placeholder="Présentez-vous, expliquez pourquoi vous êtes le bon profil…"
          disabled={applied || !isVerified}
          className="w-full rounded-xl border border-gray-200 px-3.5 py-3 text-sm text-gray-900 placeholder:text-gray-400 resize-none focus:outline-none focus:ring-2 focus:ring-[#1A6B4A]/40 focus:border-[#1A6B4A] transition-colors disabled:bg-gray-50 disabled:text-gray-400"
        />
